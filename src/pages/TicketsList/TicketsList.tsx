...
import {
  usePetrolStationsEntities,
  useLocationDataStatus,
...
const TicketsList = () => {
  ...
  const isLoading =
    ticketsStatus.isLoading ||
    referenceDataStatus.isLoading ||
    locationDataStatus.isLoading;
  ...
  const categoryTypeOptions = useMemo(
    () =>
      categoriesData.map(({ id, description }) => ({
        label: description,
        value: String(id),
      })),
    [categoriesData.length]
  );
  const ticketStatusTypeOptions = useMemo(
    () =>
      statusesData.map(({ id, description }) => ({
        label: description,
        value: String(id),
      })),
    [statusesData.length]
  );
  const busesTypeOptions = useMemo(
    () =>
      bushesData.map(({ id, description }) => ({
        label: description,
        value: String(id),
      })),
    [bushesData.length]
  ...
  const filterMeta: FilterMetaType = useMemo(
    ...
    [
      ...
      busesTypeOptions.length,
      ticketStatusTypeOptions.length,
    ]
  );
  ...
  useEffect(() => {
    ...
    referenceDataStatus.isIdle,
    ticketsStatus.isIdle,
    locationDataStatus.isIdle,
    fetchPetrolStations,
    fetchBushes,
    fetchStatusesData,
    fetchCategoriesData,
